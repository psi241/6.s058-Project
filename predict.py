...
def compute_accuracy(dataloader, name, train_prop = 0.2, knn = KNN, all_annotation_count = None):
    ...
    return pd.DataFrame({'name':name, 'k_max': k_max, 'max_acc': max_acc, 'num_tested':  len(test_vectors), 'total': all_annotation_count} | accuracy_k, index=[0])

if __name__ == "__main__":
    for trial_no, test_indices, PREDICT_MODE in [
        (2, list(range(0,109)), 'dataset'),
        (3, list(range(0,109)), 'dataset'),
    ]:
